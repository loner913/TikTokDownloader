...
- 响应乱序时，按返回记录自身的 `sec_uid` 匹配请求账号；顺序变化本身不会触发回退；
- 请求集合之外的额外记录会被忽略，其他唯一、有效的匹配继续使用；额外记录本身不会让正常账号回退；
- 缺失、重复或无效记录只会让未取得唯一、有效匹配的请求账号回退原单账号 Info，其他账号继续使用有效的预取结果；
- 整批请求异常或顶层响应结构无效时，该批已成功解析账号 ID 的账号回退原单账号 Info；
- 长链接快捷解析未命中时继续原 GET 解析；原解析流程结束后仍无法取得账号 ID，则记录该账号失败并保留任务位置及节流行为，不会在缺少 ID 时继续请求单账号 Info。

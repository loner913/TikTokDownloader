...
> [!IMPORTANT]
> 本仓库包含面向 DouK-Manager、已完成并冻结的两阶段定制提速版本。`master`、上游 Release
> 和上游下载入口不等于已验收的定制版本；分支、固定提交、构建入口、产物身份及
> 实机验收边界见 [本仓库定制版本与提速阶段](docs/performance-stages.md)。提速研发在第二阶段结束，
> 不包含第三或第四阶段。
